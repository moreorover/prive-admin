...
import type { GetHairAssignmentsToSales } from "@/modules/hair_orders/types";
import { ScrollArea, Table, Text } from "@mantine/core";

interface Props {
	hairAssignments: GetHairAssignmentsToSales;
}
...
export default function HairAssignmentToSaleTable({ hairAssignments }: Props) {
	const rows = hairAssignments.map((hairAssignment) => (
		<Table.Tr key={hairAssignment.id}>
			<Table.Td>
				<Text>{hairAssignment.hairSaleId}</Text>
			</Table.Td>
			<Table.Td>
	...
	return (
		<ScrollArea>
			<Table striped highlightOnHover>
				<Table.Thead>
					<Table.Tr>
						<Table.Th>Hair Sale ID</Table.Th>
